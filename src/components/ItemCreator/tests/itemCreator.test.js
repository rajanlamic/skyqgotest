...
describe('ItemCreator', () => {
  ...
  it('should call onToggleVisibility with the input content visibility', () => {
    const onToggleVisibility = jest.fn();
    const renderedItem = mount(
      <ItemCreator {...defaultProps} onToggleVisibility={onToggleVisibility} />
    );
    renderedItem.find('.itemCreator-button-visibility').simulate('change');
    expect(onToggleVisibility.mock.calls.length).toBe(1);
  });
